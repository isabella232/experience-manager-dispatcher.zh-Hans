...
Dispatcher 包含实现权限敏感型缓存的 AuthChecker 模块。激活模块后，Dispatcher调用AEM Servlet以对请求的内容执行用户身份验证和授权。 Servlet响应确定内容是否从缓存传送到Web浏览器。
...
1. 呈现器调用auth checker servlet以执行安全检查并响应Dispatcher。 响应消息包含 HTTP 状态代码 200 以表示用户已获得授权。
...
1. 呈现会调用AEM授权程序Servlet（这不是Dispatcher AuthChcker Servlet）以执行安全检查。 当用户获得授权时，渲染器将在响应消息的正文中包含渲染的页面。
...
1. 呈现器调用Auth Checker Servlet以执行安全检查失败，并且呈现器将原始请求转发到Dispatcher。
1. Dispatcher 将原始请求转发到渲染器。
1. 呈现会调用AEM授权程序Servlet（这不是Dispatcher AuthChcker Servlet）以执行安全检查。 当用户获得授权时，渲染器将在响应消息的正文中包含渲染的页面。
1. Dispatcher 将响应转发到浏览器。Dispatcher 将渲染器的响应消息正文添加到缓存。
...
## 创建Auth Checker Servlet {#create-the-auth-checker-servlet}
